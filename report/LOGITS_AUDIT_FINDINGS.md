...
- 扫描清单：`outputs/verification/logits_reference_inventory.csv`（**4793 条引用 / 179 个文件**；由 `python tools/audit_logits_references.py` 可重复生成，两次运行 SHA256 相同。**2026-09-17 t50 在暂存态第九次登记**：t39 与 t42 均为 4791 / 179、t35 为 4790 / 178、t32 为 4788 / 178、t28 为 4788 / 178、t23 为 4778 / 177、t21 为 4776 / 177、t20 为 4731 / 176，再早的历史登记值为 4124 条 / 153 个文件）
...
结论：**4793 条引用 / 179 个文件**（2026-09-17 t50 暂存态实测；t39 与 t42 亦为 4791 / 179、t35 为 4790 / 178、t32 与 t28 均为 4788 / 178、t23 为 4778 / 177、t21 为 4776 / 177、t20 为 4731 / 176，再早为 4124 / 153）；`kind` 分布 = `noise_float` 2775、`statement` 1220、`error_value` 798；`flag` 命中 673 条（`MIXED` 443 / 纯 247、`ORPHAN` 275 / 纯 76、`OVERCLAIM` 156 / 纯 46、`SELF_TOOL` 95 / 纯 77、`AMBIGUOUS` 6 / 纯 6、`ABS_PATH` 4 / 纯 4；同一行可带多个标记，故计数相加大于 673）。
...
连续两次运行的 CSV SHA256 相同（`D27779D6…54F8`，见第 6 节复核记录），无时间戳、无随机性。（历史值：t39 轮为 `19311E7C…1BE4`、t42 轮为 `1BE7460A…D271`、t35 轮为 `E1F708BF…1B3B`、t32 轮为 `3E46CD7B…189D`、t28 轮为 `6509CC6E…68B2`、t23 轮为 `9B4F9317…5678`、t21 轮为 `650A522C…60E3`、t20 轮为 `8EABE27A…D24C94`，更早为 `2074F146…E33B`。）
...
> 本节是 **t21 轮次** 的台账（4776 / 177）。此后 t22 又带来 **+2 / +0**，见 **§2.7**；t25/t27 又带来 **+10 / +1**，见 **§2.8**；t30 之后条数不变、校验值刷新（§2.9）；t34 又带来 **+2 / +0**（§2.10）；t41 又带来 **+1 / +1**（§2.11）；t38/t45/t46/t47 之后条数不变、校验值刷新（§2.12）；t49 又带来 **+2 / +0**，现行登记值见 **§2.13（4793 / 179）**。
...
> 本节是 **t42 轮次** 的台账（4790 → 4791）。此后 t38/t45/t46/t47 落地，条数不变、校验值刷新（§2.12）；t49 又带来 **+2 / +0**，现行登记值见 **§2.13（4793 / 179）**。
