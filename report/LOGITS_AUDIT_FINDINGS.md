...
- 扫描清单：`outputs/verification/logits_reference_inventory.csv`（**4779 条引用 / 179 个文件**；由 `python tools/audit_logits_references.py` 可重复生成，两次运行 SHA256 相同。**2026-09-19 在暂存态第十次登记**：上一轮 t50 为 4793 / 179、t39 与 t42 均为 4791 / 179、t35 为 4790 / 178、t32 为 4788 / 178、t28 为 4788 / 178、t23 为 4778 / 177、t21 为 4776 / 177、t20 为 4731 / 176，再早的历史登记值为 4124 条 / 153 个文件）
...
结论：**4779 条引用 / 179 个文件**（2026-09-19 暂存态实测；上一轮 t50 为 4793 / 179、t39 与 t42 亦为 4791 / 179、t35 为 4790 / 178、t32 与 t28 均为 4788 / 178、t23 为 4778 / 177、t21 为 4776 / 177、t20 为 4731 / 176，再早为 4124 / 153）；`kind` 分布 = `noise_float` 2761、`statement` 1220、`error_value` 798；`flag` 命中 672 条（`MIXED` 442 / 纯 248、`ORPHAN` 273 / 纯 76、`OVERCLAIM` 154 / 纯 46、`SELF_TOOL` 95 / 纯 77、`AMBIGUOUS` 6 / 纯 6、`ABS_PATH` 4 / 纯 4；同一行可带多个标记，故计数相加大于 672）。
...
连续两次运行的 CSV SHA256 相同（`899F97B6…BC45`，见第 6 节复核记录），无时间戳、无随机性。（历史值：t50 轮为 `D27779D6…54F8`、t39 轮为 `19311E7C…1BE4`、t42 轮为 `1BE7460A…D271`、t35 轮为 `E1F708BF…1B3B`、t32 轮为 `3E46CD7B…189D`、t28 轮为 `6509CC6E…68B2`、t23 轮为 `9B4F9317…5678`、t21 轮为 `650A522C…60E3`、t20 轮为 `8EABE27A…D24C94`，更早为 `2074F146…E33B`。）
...
> 本节是 **t21 轮次** 的台账（4776 / 177）。此后 t22 又带来 **+2 / +0**，见 **§2.7**；t25/t27 又带来 **+10 / +1**，见 **§2.8**；t30 之后条数不变、校验值刷新（§2.9）；t34 又带来 **+2 / +0**（§2.10）；t41 又带来 **+1 / +1**（§2.11）；t38/t45/t46/t47 之后条数不变、校验值刷新（§2.12）；t49 又带来 **+2 / +0**，现行登记值见 **§2.14（4779 / 179）**。
...
> 本节是 **t42 轮次** 的台账（4790 → 4791）。此后 t38/t45/t46/t47 落地，条数不变、校验值刷新（§2.12）；t49 又带来 **+2 / +0**，现行登记值见 **§2.14（4779 / 179）**。
...
### 2.12 t39 第八次登记：t38+t45+t46+t47 落地后的校验值刷新（2026-09-17，暂存态）

本轮（t39）登记 t38（F-1 死路径 + F-2 白名单缺口，类别级）、t45（F-11 的 PPT 侧措辞 + 重建 PPTX 与 PPT-PDF）、t46（F-11 的报告侧措辞 + 重建 `REPORT.docx` 与 `report.pdf`）、t47（F-12 四类断言盲区）四轮落地后的暂存态：**条数 / 文件数 +0，但校验值已变**（4,792 行；1,246,850 → **1,246,912 B**；`1BE7460A…D271` → **`19311E7C…1BE4`**）。逐来源对账：

| 来源 | 条数 | 文件数 | 说明 |
|---|---|---|---|
| `tools/update_defense.py` | 0 | 0 | 114 → 114；**21 行被扫内容变化**（t41 轴范围改现读 + t45 措辞） |
| `tools/selfcheck.py` | 0 | 0 | 11 → 11；**11 行被扫内容变化**（t41/t47 断言） |
| `report/REQUIREMENTS_AUDIT.md` | 0 | 0 | 158 → 158；1 行被扫内容变化（t38 的 SVG 路径改正与历史标注） |
| 其余全部文件 | 0 | 0 | 逐文件相减无差异（含 `report/SYNC_NOTES.md`、`report/PPT_CONTENT.md`、PPT 三件、报告三件、`tools/check_audit_evidence.py` 与重生后的自检报告 JSON：受扫行内容均未变） |
| **合计** | **0** | **0** | 4791 → 4791（被扫行内容变化合计 **33 行 / 3 个文件**） |

**本轮的两项「必做」**：① 入库的 `outputs/metrics/selfcheck_report.json` 在最终树上重跑刷新（9,214 B / `generated_at = 2026-09-17T18:19:38` / 34 项全 PASS / 可视化项标题含「图表数据落在轴范围内 + 图元非空白 + PPTX↔PDF 同源」）并 `git add`；② `report/SYNC_NOTES.md` §14.3 的两行（F-1/F-2）已从「未修 + 归因 escalated」改为「t38 已闭合 + 根因 = 依赖误挂在 failed 的 t36 上」，同型陈旧句一并标注（细节见 §14.3）。
...
### 2.13 t50 第九次登记：t49 的 F-13/F-15 修复后的增量（2026-09-17，暂存态）

本轮（t50）登记 t49（F-13：把「无显式轴边界」从提示项升级为机检 FAIL；F-15：把 `REPORT.docx` / `ppt_svg` 15 份 / 工作流程图两件 / `SPEC13_FREEZE.json` 四类 in-repo 载体并入既有提交项闸门）之后的暂存态：**+2 条 / +0 个文件**（4791 → **4793**），清单 4,794 行；校验值 1,246,912 → **1,247,351 B**、`19311E7C…1BE4` → **`D27779D6…54F8`**。逐来源对账：

| 来源 | 条数 | 文件数 | 说明 |
|---|---|---|---|
| `tools/selfcheck.py` | **+2** | 0 | 11 → 13：t49 新增的两处被扫文本（F-13 的机检文案、F-15 的载体清单/关键节点说明）各记 1 条 |
| 其余全部文件 | 0 | 0 | 逐文件相减 0，**且 179 个文件的被扫行内容变化合计 = 0**（重生后的入库自检报告 JSON 的 6 行在扫、摘录未变；PPT 三件、报告三件、`tools/update_defense.py`、`tools/check_audit_evidence.py` 均无变化） |
| **合计** | **+2** | **0** | 4791 → 4793（`statement` 1218 → **1220**；`error_value` 798、`noise_float` 2775 不变） |

**性质：内容级变化（新增断言与文案），非劣化** —— 旁证：三门禁全绿、自检 34/34、`check_report_assets --strict` OK=177/FAIL=0、可重算断言保持 **134**、路径引用 **568**、§引用 **235**。**本轮另在契约外补做一处**：校正 `report/SYNC_NOTES.md` §16.4 里 t39 时点的可视化项明细串（随 t49 过期），并在最终树重生 `outputs/metrics/selfcheck_report.json`（9,500 B / `generated_at = 2026-09-17T19:20:15` / 34 项全 PASS），其「图表轴范围 OK（8 图 / 9 条数值轴全部显式设边界）」与提交项载体明细均与当前实现逐项一致。**未运行** `tools/update_defense.py`；**未触碰**答辩 PPT 第 8 页第 5 图。
...
### 2.14 t? 第十次登记：插图落地 + 用户保存 PPT 之后的增量（2026-09-19，暂存态）

> 本节为**追加登记**，不改动 §2.1–§2.13 的任何历史记录。为不扰动既有台账的行号定位，本节**追加在文末**。

本轮登记值：**4779 条 / 179 个文件**（上一轮 t50 为 4793 / 179）；清单 4,780 行、1,243,467 B、SHA256 `899F97B6…BC45`（上一轮 `D27779D6…54F8`、1,247,351 B）。逐文件对账：

| 来源 | 条数 | 文件数 | 说明 |
|---|---|---|---|
| `report/答辩PPT_RepViT.pptx` | **−14** | 0 | 95 → 81：2026-09-19 用户在 PowerPoint 中保存所致（内部部件换号 + 第 3 页删除 3 字 `一句话：`，无其它内容变化） |
| `report/REPORT.md` | 0 | 0 | 56 → 56：t2 插入 12 张插图（源文 +24 行）使行号下移，仅 location 重排 |
| `report/REPORT.docx` | 0 | 0 | 56 → 56：同上，段号重排 |
| `report.pdf` | 0 | 0 | 57 → 57：同上，页码 26 → 31 |
| `report/REQUIREMENTS_AUDIT.md` | 0 | 0 | 158 → 158：本轮修正 2 处体积断言 + 13 条行号指针 |
| 本目录第三份登记文档 | 0 | 0 | 215 → 215：本轮补 5 处历史标注 |
| **合计** | **−14** | **0** | 4793 → 4779 |

**性质：可归因的载体变化（用户保存 PPT + 插图版式重排），非口径劣化。** `kind` 仅 `noise_float` 2775 → 2761；`flag` 标记行 673 → 672。
全部台账与边界见 `report/SYNC_NOTES.md` §18。

### 2.15 第十次登记的复审追加修订（2026-09-19）

复用复审修正 `report/REQUIREMENTS_AUDIT.md` 的 3 处确证错误：① **4-A10** 的括注原写「温度缩放可显著降低」与报告正文矛盾（报告 WT[472-475] 明确只落盘 `ece_before`、未落盘校准后的 ECE，故不作量化断言），已改为与正文一致的表述并把引用重定位到 `:472-475`；② **A1-9** 的推荐得分 `0.9842` 无据（该值全仓只出现在预测置信度类 CSV），按 `outputs/advanced/model_recommendation.csv` 实测改为 **`0.9792`** 并标注来源，引用重定位到 `:186-188`；③ **L155 / L728** 的 3 处简写行号指针（`:773`、`:435`、`:773`）重定位为 `:795`、`:457`/`:795`（目标行均含关键 token「90.32」）。

这 4 行不在扫描命中集内 ⇒ 清单**逐字节不变**（4779 条 / 179 个文件、4,780 行、1,243,467 B、SHA256 `899F97B6…BC45`），§2.14 的登记值继续有效。
