...
扫描清单：outputs/verification/logits_reference_inventory.csv（**2026-09-19 第十次登记：4779 条引用 / 179 个文件**；上一轮 t50 为 4793 / 179、t39 与 t42 为 4791 / 179、t42 为 4791 / 179、t35 为 4790 / 178、t32 为 4788 / 178、t28 为 4788 / 178、t23 为 4778 / 177、t21 为 4776 / 177、t20 为 4731 / 176，更早的历史值为 4124 / 153），覆盖Git跟踪的文本、代码、JSON、日志、SVG，以及PPTX/DOCX/PDF正文与PPT备注；核对请按该表的 `bucket` 列查。原始实验产物不做数字替换；旧SVG演示源保留为历史素材，不作为答辩口径。
（第十次登记（2026-09-19）增量 −14 / +0：全部来自 `report/答辩PPT_RepViT.pptx`（95 → 81，用户保存导致的部件换号，见 `report/SYNC_NOTES.md` §18.4）；`report/REPORT.md` / `report/REPORT.docx` / `report.pdf` 各 56 / 56 / 57 → 同数，仅 location 随插图整体位移（+50 / −50、+53 / −53、+52 / −52）；本轮另修正 `report/REQUIREMENTS_AUDIT.md` 两处体积断言与 13 条行号指针（+4 / −4）、本目录第三份登记文档 5 处历史标注（+2 / −2）。行数 4,794 → **4,780**、字节 1,247,351 → **1,243,467**、SHA256 `D27779D6…54F8` → **`899F97B6…BC45`**。）
（t50 增量 +2 / +0：两处新增条目全部来自 `tools/selfcheck.py`（11 → 13）—— t49 的 F-13 机检文案与 F-15 载体清单各记 1 条；其余 178 个文件逐文件相减 0，**且 179 个文件的被扫行内容变化合计 = 0**（重生后的自检报告 JSON 的 6 行在扫、摘录未变）。字节 1,246,912 → **1,247,351**、SHA256 `19311E7C…1BE4` → **`D27779D6…54F8`**。）
（t39 增量 +0 / +0：条数与文件数都与 t42 相同，但校验值已变 —— t38/t45/t46/t47 四轮改动使 **33 行 / 3 个文件**的被扫内容变化：`tools/update_defense.py` 21（轴范围现读 + 措辞）、`tools/selfcheck.py` 11（t41/t47 断言）、`report/REQUIREMENTS_AUDIT.md` 1（SVG 路径与标注）；另有重生后的自检报告 JSON、报告三件、PPT 三件与 `tools/check_audit_evidence.py` 的受扫行内容未变。字节 1,246,850 → **1,246,912**、SHA256 `1BE7460A…D271` → **`19311E7C…1BE4`**。）
...
（第十次登记·复审追加修订（2026-09-19）：复用复审修正了 `report/REQUIREMENTS_AUDIT.md` 的 3 处确证错误（4-A10 的括注与引用、A1-9 的推荐得分 0.9842 → 0.9792 与引用、两处简写行号指针）。这 4 行不在扫描命中集内，故清单**逐字节不变**：仍为 4779 条引用 / 179 个文件、4,780 行、1,243,467 B、SHA256 `899F97B6…BC45`。明细见 `report/SYNC_NOTES.md` §19。）
